...
```r
round((59 + 73 + 2) / 3) # round off: 반올림하다.
```
...
```r
y <- seq(from = 1, to = 10, length.out = 5) # sequence: 수열
y
```
...
```r
(y <- seq(1, 10, length.out = 5)) # surround the assignment with parentheses
```
...
여기서는 간단한 예시로 변수 이름을 만드는 방법에 대해서 알아본다.

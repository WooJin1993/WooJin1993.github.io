...
def solution(progresses, speeds):
    """
    완성된 기간 = 남은 작업 진도 (100 - p)에서 작업 속도 s를 나눈 값의 올림
    """
    ...
    # itertools 모듈의 groupby 함수를 이용하여 각 누적 최댓값이 몇 번 발생했는지 반환
    return [len(list(group)) for _, group in groupby(publish_days)] 
```